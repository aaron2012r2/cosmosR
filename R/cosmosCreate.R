...
cosmosCreate <- function(sql.json = "", max.items = 100, debug.auth = TRUE, debug.query = TRUE, content.response = FALSE) {
    ...
    raw.response <- POST(post.uri, add_headers(.headers = c("Authorization" = auth.header, "x-ms-version" = "2017-02-22", "x-ms-date" = ms.date.string, "Content-Type" = "application/json")), body = '{"batch_id_ngi" : "ok_cosmosdb_test"}')
